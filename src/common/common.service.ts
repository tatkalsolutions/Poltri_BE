...
DECLARE @tempGodown TABLE (CODE INT);
INSERT INTO @tempGodown 
SELECT CODE 
FROM MSTCOMMGODOWN 
WHERE STATUS_CODE = 0;
...
SELECT 
    ...
    dbo.FORMAT_DATE(TRNMATPOST.TRAN_DATE) AS PRN_TRANDATE,
    ...
    NULL AS OP_STK, NULL AS TODAYS_TOTAL, NULL AS USE_STOCK_KG, NULL AS CL_STK
FROM TRNMATPOST 
...
WHERE TRNMATPOST.STATUS_CODE = 0 
  AND TRNACCTMATH.TRAN_DATE BETWEEN @startDate AND @toDate
  AND SUBSTRING(CAST(TRNMATPOST.TRAN_NO AS VARCHAR(50)), 8, 5) = '20215'
...
WHERE TRNMATPOST.STATUS_CODE = 0 
  AND TRNACCTMATH.TRAN_DATE BETWEEN @startDate AND @toDate
  AND SUBSTRING(CAST(TRNMATPOST.TRAN_NO AS VARCHAR(50)), 8, 5) IN ('11514','11543')
...
SELECT 
    ...
    CASE WHEN MAT_CODE = 1010300014 THEN QTY ELSE NULL END AS LAYER_MASH, 
    CASE WHEN MAT_CODE = 1010300012 THEN QTY ELSE NULL END AS GROWER_MASH,

    OpeningStock.OP_STK,
    TodayTotal.TODAYS_TOTAL_UTPADAN,
    UsedStock.USE_STOCK_KG,

    TodayTotal.TODAYS_TOTAL_UTPADAN - UsedStock.USE_STOCK_KG AS CL_STK
...
CROSS APPLY (
    SELECT 
        ISNULL((SELECT SUM(QTY)
                FROM TRNMATPOST ST2
                WHERE ST2.TRAN_DATE < TRNMATPOST.TRAN_DATE
                  AND ST2.STATUS_CODE IN (0,21)
                  AND ST2.MAT_CODE = TRNMATPOST.MAT_CODE
                  AND ST2.GODOWN_CODE IN (SELECT CODE FROM @tempGodown)
        ),0)
        +
        ISNULL((SELECT SUM(QTY)
                FROM TRNMATPOST ST3
                WHERE ST3.TRAN_DATE = TRNMATPOST.TRAN_DATE
                  AND ST3.STATUS_CODE IN (0,21)
                  AND ST3.MAT_CODE = TRNMATPOST.MAT_CODE
                  AND SUBSTRING(CONVERT(VARCHAR, ST3.TRAN_NO), 8, 5) IN ('30211')
                  AND ST3.GODOWN_CODE IN (SELECT CODE FROM @tempGodown)
        ),0) AS OP_STK
) OpeningStock
...
CROSS APPLY (
    SELECT 
        ...
        ISNULL((SELECT SUM(QTY)
                FROM TRNMATPOST ST4
                WHERE ST4.TRAN_DATE = TRNMATPOST.TRAN_DATE
                  AND ST4.STATUS_CODE IN (0,21)
                  AND ST4.MAT_CODE = TRNMATPOST.MAT_CODE
                  AND SUBSTRING(CONVERT(VARCHAR, ST4.TRAN_NO), 8, 5) IN ('55025','20215')
                  AND ST4.GODOWN_CODE IN (SELECT CODE FROM @tempGodown)
        ),0)
        +
        ISNULL((SELECT SUM(QTY)
                FROM TRNMATPOST ST5
                WHERE ST5.TRAN_DATE = TRNMATPOST.TRAN_DATE
                  AND ST5.STATUS_CODE IN (0,21)
                  AND ST5.MAT_CODE = TRNMATPOST.MAT_CODE
                  AND SUBSTRING(CONVERT(VARCHAR, ST5.TRAN_NO), 8, 5) IN ('55045')
                  AND ST5.GODOWN_CODE IN (SELECT CODE FROM @tempGodown)
        ),0) AS TODAYS_TOTAL_UTPADAN
) TodayTotal

-- USED STOCK
CROSS APPLY (
    SELECT ISNULL((SELECT SUM(QTY)
                   FROM TRNMATPOST STMP
                   WHERE STMP.TRAN_DATE = TRNMATPOST.TRAN_DATE
                     AND STMP.STATUS_CODE IN (0,21)
                     AND STMP.MAT_CODE = TRNMATPOST.MAT_CODE
                     AND SUBSTRING(CONVERT(VARCHAR, STMP.TRAN_NO), 8, 5) = '55045'
                     AND STMP.GODOWN_CODE IN (SELECT CODE FROM @tempGodown)
                     AND STMP.GODOWN_CODE <> 101001
    ),0) AS USE_STOCK_KG
) UsedStock

WHERE TRNMATPOST.STATUS_CODE = 0 
  AND TRNACCTMATH.TRAN_DATE BETWEEN @startDate AND @toDate
  AND SUBSTRING(CAST(TRNMATPOST.TRAN_NO AS VARCHAR(50)), 8, 5) = '55025'
