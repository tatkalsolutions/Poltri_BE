...
SELECT 
	...
	-- Opening stock (material wise, date wise)
    OPENING_STK.OP_STK AS OP_STK,
	-- Closing stock (material wise, date wise)
    CLOSING_STK.CL_STK AS CL_STK
FROM TRNMATPOST 
	LEFT JOIN TRNACCTMATH ON TRNACCTMATH.TRAN_NO = TRNMATPOST.TRAN_NO
	LEFT JOIN MSTMATERIALS ON MSTMATERIALS.CODE = TRNMATPOST.MAT_CODE
	-- Closing Stock calculation
	CROSS APPLY(
		SELECT SUM(QTY) AS OP_STK FROM TRNMATPOST TMP WHERE TMP.MAT_CODE = TRNMATPOST.MAT_CODE AND TMP.STATUS_CODE = 0 AND TMP.TRAN_DATE < TRNMATPOST.TRAN_DATE
	) AS OPENING_STK
	-- Closing Stock calculation
	CROSS APPLY(
		SELECT SUM(QTY) AS CL_STK FROM TRNMATPOST TMP WHERE TMP.MAT_CODE = TRNMATPOST.MAT_CODE AND TMP.STATUS_CODE = 0 AND TMP.TRAN_DATE <= TRNMATPOST.TRAN_DATE
	) AS CLOSING_STK
